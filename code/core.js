let getz = B => {
	...
	let z = { 
		dimensions: {
			width, height, min, max, version, v
		},
		highcontrast: false,
		texthidden: false,
		animate: true,
	...
	z.tools = {
		getdimensions: () => {
			...
			let version = (min < 680 || max < 980) ? "small" : "large";
			let v = version === "small" ? 0 : 1;
			return { width, height, min, max, version, v };
		},
		...
		applyStyles: ({el, css=[], attributes=[], cssclasses=[], notcssclasses=[], cssstyles=[]}) => {
			if(el && z.tools.isDom(el)) {
 ...
 */
	...
	//set controls
	( () => {
		window.addEventListener("resize", evt => { z.dimensions = z.tools.getdimensions });
